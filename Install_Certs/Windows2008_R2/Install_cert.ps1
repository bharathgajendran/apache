...
4.0 - Added support for installing PFX personal certificates
5.0 - Added support for installing p7b personal certificates
#>	
 ...
 foreach ($cert in $certFile)
    {
		...
		certutil.exe -Repairstore "my" "$certhash" >cert.txt
        Write-Host "Certificate" $certfile.Name[$m] "- Installed SUCCESSFULLY!" 
...
foreach ($cert in $certFile)
    {
		certreq.exe -Accept -machine  $cert.FullName
        $readurl = Read-Host "Enter the URL as shown in (issued to) of the certificate"
	    Get-ChildItem $location -Recurse | where { $_.subject -match ($readurl) } | Format-list Thumbprint, Subject, NotAfter
	    $certhash = Read-host "Enter the tumbprint of the certificate matching your URL and expiry date"
...
elseif (($certFile -ne $NULL) -and ($l8 -eq 1))
  {
    certreq.exe -Accept -machine  $certfile.Fullname
	$readurl = Read-Host "Enter the URL as shown in (issued to) of the certificate"
	Get-ChildItem Cert:/Localmachine/My -Recurse | where { $_.subject -match ($readurl) } | Format-list Thumbprint, Subject, NotAfter
	$certhash = Read-host "Enter the tumbprint of the certificate matching your URL and expiry date"
    $pref = Read-Host "Do you want to assign certificate to Default website (y/n)"
    if ($pref -eq 'y')
    {
