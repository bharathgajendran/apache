...
foreach ($site in $sites){
    ...
    $certs = Get-ChildItem IIS:\SslBindings | ? {
         ...
         Foreach ($cert in $certs){
            $line = Get-ChildItem Cert:\LocalMachine\My | Where {$_.Thumbprint -eq $Cert -and $_.NotAfter -lt $expirationDate}  | Select @{N="SiteName";E={$site}},Subject, @{N="Subject Alternative Name";E={($_.extensions | where {$_.Oid.friendlyname -eq "Subject Alternative Name"}).format(1)}}, NotAfter, Issuer
            $report += $line 
...
-From "$env:COMPUTERNAME@toyota.com" -To "youremailaddress.com" `
